var player ;
//播放列表
var mplayer_list = new Array();
$(document).ready(function() {
	var modeText = ['顺序播放', '单曲循环', '随机播放', '列表循环'];
	var mplayer_list = initMusicList();
	player = new MPlayer({
		// 容器选择器名称
		containerSelector: '.mp',
		// 播放列表
		songList: mplayer_list,
		// 专辑图片错误时显示的图片
...
//		console.log('第'+ (i+1) +'条数据的键值为：' + localStorage.key(i) +'，数据为：' + localStorage.getItem(localStorage.key(i)));
		value = eval('(' + localStorage.getItem(localStorage.key(i)) + ')');
		mplayer_song.push(value.data)
	}			
	
	mplayer_list[0]=mplayer_song;
	return mplayer_list;
}
...
//		console.log('第'+ (i+1) +'条数据的键值为：' + localStorage.key(i) +'，数据为：' + localStorage.getItem(localStorage.key(i)));
	...
	player.settings.songList[0] = mplayer_song;
}
